...
export async function runDLQHealthCheck(env: Env): Promise<void> {
  ...
  try {
    ...
    if (dlqCount > DLQ_ALERT_THRESHOLD) {
      console.error(`[dlq-health] ALERT: DLQ has ${dlqCount} messages (threshold: ${DLQ_ALERT_THRESHOLD})`);
    }
    ...
    const expiredCount = Number((expiredResult as Record<string, unknown>).c ?? 0);
    if (expiredCount > 0) {
      console.log(`[dlq-health] ${expiredCount} expired DLQ messages (>7 days)`);
    }
