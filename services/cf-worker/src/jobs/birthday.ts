...
export async function runBirthdayJob(env: Env): Promise<void> {
  ...
  try {
    ...
    for (const user of birthdayUsers) {
      ...
      const firstName = String((user as Record<string, unknown>).first_name || 'Someone');
      ...
      try {
        ...
        ).bind(birthdayUserId, birthdayUserId, birthdayUserId).all();

        const matchIds = (matches ?? []).map((m) => String((m as Record<string, unknown>).match_user_id));
        console.log(`[birthday] ${firstName} has ${matchIds.length} mutual match(es)`);
        ...
        for (const matchUserId of matchIds) {
          try {
            ...
            if (response.ok) {
              console.log(`[birthday] Notified ${matchUserId} about ${firstName}'s birthday`);
            } else {
              console.error(`[birthday] Failed to notify ${matchUserId}: ${response.status}`);
            }
          } catch (error) {
