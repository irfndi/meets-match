...
export const matchesCommand = (ctx: Context) =>
  Effect.gen(function* (_) {
    ...
    // Process up to 10 matches
    const matchesToProcess = matches.slice(0, 10);

    // ⚡ Bolt: Prevent N+1 query bottleneck by batching user profile fetches concurrently.
    // Fetching profiles in an unbounded concurrent batch reduces the time it takes
    // to render the match list significantly compared to sequential `yield* _` calls.
    const userFetchEffects = matchesToProcess.map((match) => {
      const otherUserId = match.user1Id === userId ? match.user2Id : match.user1Id;
      return userService.getUser(otherUserId).pipe(
        Effect.map((res) => ({ match, otherUserId, user: res.user })),
        Effect.catchAll((e) => {
    ...
    const results = yield* _(Effect.all(userFetchEffects, { concurrency: 'unbounded' }));

    for (let i = 0; i < results.length; i++) {
      const result = results[i];
      if (!result || !result.user) continue;

      const { match, otherUserId, user } = result;
