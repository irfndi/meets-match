...
export const matchesCommand = (ctx: Context) =>
  Effect.gen(function* (_) {
    ...
    // ⚡ Bolt: Fetch user profiles concurrently to fix N+1 API calls
    // Using Effect.all with concurrency to run independent requests in parallel
    const matchUsersEffects = topMatches.map((match) => {
      const otherUserId = match.user1Id === userId ? match.user2Id : match.user1Id;
      return userService.getUser(otherUserId).pipe(
        Effect.map((userRes) => ({ match, otherUser: userRes.user, otherUserId })),
        Effect.catchAll((e) => {
    ...
    const matchUsers = yield* _(Effect.all(matchUsersEffects, { concurrency: 'unbounded' }));

    for (let i = 0; i < matchUsers.length; i++) {
      const result = matchUsers[i];
