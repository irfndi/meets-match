...
import { handleReportConversation, handleLikeMessageConversation, handleLikeMessageMedia } from '../handlers/match.js';
...
export async function checkAndUpdateProfileComplete(env: Env, userId: string): Promise<boolean> {
  try {
...
export async function handleConversationMessage(ctx: MyContext, env: Env): Promise<boolean> {
  ...
  if (text === t('genericCancel', lang)) {
    ...
    await ctx.reply(t('genericCancelled', lang), { reply_markup: getMainMenuKeyboard() });
    return true;
  }
...
export async function handleContactMessage(ctx: MyContext, env: Env): Promise<boolean> {
  ...
  if (String(contact.user_id) !== userId) {
    await ctx.reply('Please share your own contact.', { reply_markup: getMainMenuKeyboard() });
    return true;
  }

  const phoneNumber = contact.phone_number;
  if (!phoneNumber) {
    await ctx.reply('Could not get phone number. Please try again.', { reply_markup: getMainMenuKeyboard() });
    return true;
  }
...
async function reverseGeocodeLocation(env: Env, latitude: number, longitude: number): Promise<{ city?: string; country?: string } | null> {
  try {
    const res = await env.API_SERVICE.fetch(
      new Request(`http://api/geocode?lat=${latitude}&lon=${longitude}`)
    );
    if (!res.ok) return null;
    const data = await res.json() as { result?: { city?: string; country?: string } };
...
export async function handleLocationMessage(ctx: MyContext, env: Env): Promise<boolean> {
  ...
  if (success) {
    ...
    await ctx.reply(t('locationUpdated', 'en'), { reply_markup: getMainMenuKeyboard() });
    if (becameComplete) {
      await promptPhoneVerification(ctx, env, 'en');
    }
  } else {
    await ctx.reply(t('genericError', 'en'), { reply_markup: getMainMenuKeyboard() });
  }
  return true;
}
...
export async function handleMediaMessage(
  ...
  if (currentCount >= 3) {
    await ctx.reply(t('mediaMaxReached', lang, { count: String(currentCount) }));
    return true;
  }
  ...
  try {
    ...
    if (!file.file_path) {
      console.error('[media] Failed to get file path from Telegram for file_id:', fileId);
    ...
    if (!response.ok) {
      console.error('[media] Failed to download from Telegram:', response.status, response.statusText);
      await ctx.reply(t('mediaUploadError', lang));
      return true;
    }
    ...
    console.log(`[media] Downloaded ${fileType} from Telegram: ${bytes.length} bytes`);
    ...
    if (!apiResponse.ok) {
      ...
      console.error(`[media] API register failed: ${apiResponse.status} ${errorBody}`);
      ...
      if (apiResponse.status === 429) {
        ...
        await ctx.reply(t('mediaLimitReached', lang), { parse_mode: 'Markdown', reply_markup: keyboard });
        return true;
      }
      ...
      await ctx.reply(t('mediaRetryPrompt', lang), { reply_markup: retryKeyboard });
      return true;
    }
...
async function handleBioConversation(ctx: MyContext, env: Env, state: ConversationState, text: string, lang: Language): Promise<boolean> {
  ...
  if (success) {
    const becameComplete = await checkAndUpdateProfileComplete(env, state.userId);
    await ctx.reply(t('bioUpdated', lang), { reply_markup: getMainMenuKeyboard() });
    if (becameComplete) await promptPhoneVerification(ctx, env, lang);
  } else {
...
async function handleBirthDateConversation(ctx: MyContext, env: Env, state: ConversationState, text: string, lang: Language): Promise<boolean> {
  ...
  if (success) {
    const becameComplete = await checkAndUpdateProfileComplete(env, state.userId);
    await ctx.reply(t('birthDateUpdated', lang), { reply_markup: getMainMenuKeyboard() });
    if (becameComplete) await promptPhoneVerification(ctx, env, lang);
  } else {
    await ctx.reply(t('genericError', lang), { reply_markup: getMainMenuKeyboard() });
  }
...
async function handleNameConversation(ctx: MyContext, env: Env, state: ConversationState, text: string, lang: Language): Promise<boolean> {
  ...
  if (success) {
    const becameComplete = await checkAndUpdateProfileComplete(env, state.userId);
    await ctx.reply(t('nameUpdated', lang, { name }), { reply_markup: getMainMenuKeyboard() });
    if (becameComplete) await promptPhoneVerification(ctx, env, lang);
  } else {
    await ctx.reply(t('genericError', lang), { reply_markup: getMainMenuKeyboard() });
  }
...
async function handleGenderConversation(ctx: MyContext, env: Env, state: ConversationState, text: string, lang: Language): Promise<boolean> {
  ...
  if (success) {
    const becameComplete = await checkAndUpdateProfileComplete(env, state.userId);
    await ctx.reply(t('genderUpdated', lang), { reply_markup: getMainMenuKeyboard() });
    if (becameComplete) await promptPhoneVerification(ctx, env, lang);
  } else {
    await ctx.reply(t('genericError', lang), { reply_markup: getMainMenuKeyboard() });
  }
...
async function handleInterestsConversation(ctx: MyContext, env: Env, state: ConversationState, text: string, lang: Language): Promise<boolean> {
  ...
  if (success) {
    const becameComplete = await checkAndUpdateProfileComplete(env, state.userId);
    await ctx.reply(t('interestsUpdated', lang, { interests: interests.join(', ') }), { reply_markup: getMainMenuKeyboard() });
    if (becameComplete) await promptPhoneVerification(ctx, env, lang);
  } else {
    await ctx.reply(t('genericError', lang), { reply_markup: getMainMenuKeyboard() });
  }
...
async function handleAgeRangeConversation(ctx: MyContext, env: Env, state: ConversationState, text: string, lang: Language): Promise<boolean> {
  ...
  if (rangeMatch) {
    ...
    await clearConversationState(env.KV, state.userId);
    if (success) {
      await ctx.reply(t('ageRangeUpdated', lang, { min: String(min), max: String(max) }), { reply_markup: getMainMenuKeyboard() });
    } else {
      await ctx.reply(t('genericError', lang), { reply_markup: getMainMenuKeyboard() });
    }
    return true;
  ...
  if (step === 0) {
    ...
    await ctx.reply(t('ageRangeSelectMax', lang, { min: String(val) }), { reply_markup: getMainMenuKeyboard() });
    return true;
  }
  ...
  await clearConversationState(env.KV, state.userId);
  if (success) {
    await ctx.reply(t('ageRangeUpdated', lang, { min: String(min), max: String(val) }), { reply_markup: getMainMenuKeyboard() });
  } else {
    await ctx.reply(t('genericError', lang), { reply_markup: getMainMenuKeyboard() });
  }
  return true;
...
async function handleDistanceConversation(ctx: MyContext, env: Env, state: ConversationState, text: string, lang: Language): Promise<boolean> {
  ...
  await clearConversationState(env.KV, state.userId);
  if (success) {
    await ctx.reply(t('distanceUpdated', lang, { distance: String(distance) }), { reply_markup: getMainMenuKeyboard() });
  } else {
    await ctx.reply(t('genericError', lang), { reply_markup: getMainMenuKeyboard() });
  }
  return true;
...
async function handleGenderPrefConversation(ctx: MyContext, env: Env, state: ConversationState, text: string, lang: Language): Promise<boolean> {
  ...
  await clearConversationState(env.KV, state.userId);
  if (success) {
    await ctx.reply(t('genderPrefUpdated', lang, { preferences: normalized.join(', ') }), { reply_markup: getMainMenuKeyboard() });
  } else {
    await ctx.reply(t('genericError', lang), { reply_markup: getMainMenuKeyboard() });
  }
  return true;
