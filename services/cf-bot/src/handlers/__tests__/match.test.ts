...
describe("Match Handlers", () => {
  ...
  describe("matchCommand", () => {
    it("should show finding matches for complete user", async () => {
      ...
      expect(ctx.reply).toHaveBeenCalledWith(expect.stringContaining("Finding"), expect.anything());
    });
    ...
    it("should show no matches when potential matches is empty", async () => {
      ...
      await matchCommand(ctx, env);
      expect(ctx.reply).toHaveBeenCalledWith(expect.stringContaining("No potential matches"), expect.anything());
    });

    it("should show match cards when potential matches exist", async () => {
      env.API_SERVICE = createMockApiService({
        "/users/123": () => new Response(JSON.stringify({ user: completeUser }), { status: 200 }),
        "/potential-matches": () => new Response(JSON.stringify({ potentialMatches: [
          { id: "456", displayName: "Alice", age: 24, bio: "Hello", gender: "female", interests: ["Reading"], location: { city: "Jakarta", country: "Indonesia" } },
        ] }), { status: 200 }),
      });
      await matchCommand(ctx, env);
    ...
    it("should prompt phone verification for unverified user", async () => {
      ...
      env.API_SERVICE = createMockApiService({
        "/users/123": () => new Response(JSON.stringify({ user: userWithoutPhone }), { status: 200 }),
      });
      await matchCommand(ctx, env);
      expect(ctx.reply).toHaveBeenCalledWith(expect.stringContaining("phone"), expect.anything());
    });
  });
  ...
  describe("matchCallbacks", () => {
    ...
    it("should handle match:dislike callback", async () => {
      ...
      env.API_SERVICE = createMockApiService({
        "/users/123": () => new Response(JSON.stringify({ user: completeUser }), { status: 200 }),
        "/matches": () => new Response(JSON.stringify({ match: { id: "m1", user1Id: "123", user2Id: "456", status: "PENDING" } }), { status: 201 }),
        "/matches/m1/dislike": () => new Response(JSON.stringify({ id: "m1", status: "REJECTED" }), { status: 200 }),
      });
      await matchCallbacks(ctx, env);
      expect(ctx.reply).toHaveBeenCalled();
    });
    ...
    it("should handle match:skip callback", async () => {
      ...
      env.API_SERVICE = createMockApiService({
        "/users/123": () => new Response(JSON.stringify({ user: completeUser }), { status: 200 }),
        "/matches": () => new Response(JSON.stringify({ match: { id: "m1", user1Id: "123", user2Id: "456", status: "PENDING" } }), { status: 201 }),
        "/matches/m1/skip": () => new Response(JSON.stringify({ id: "m1", status: "PENDING" }), { status: 200 }),
      });
      await matchCallbacks(ctx, env);
      expect(ctx.reply).toHaveBeenCalled();
    });
