  ...
  try {
    ...
    if (data.startsWith("media:delete:")) {
      ...
      if (freshMedia.length === 0) {
        const keyboard = new InlineKeyboard()
          .text(t("profileUploadMedia", lang), "media:upload")
          .row()
          .text(t("profileBackToProfile", lang), "media:back");
        await ctx.editMessageText(t("mediaManagerEmpty", lang), {
          parse_mode: "Markdown",
          reply_markup: keyboard,
        });
        ...
        if (freshMedia.length < 3) {
          keyboard.text(t("profileUploadMore", lang), "media:upload");
        }
        keyboard.row().text(t("profileBackToProfile", lang), "media:back");
        await ctx.editMessageText(msg, {
          parse_mode: "Markdown",
          reply_markup: keyboard,
        });
