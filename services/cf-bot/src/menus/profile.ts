...
export async function handleProfileCallback(ctx: MyContext, env: Env, data: string): Promise<boolean> {
  ...
  switch (data) {
    ...
    case 'profile:media': {
      const mediaUserRes = await env.API_SERVICE.fetch(new Request(`http://api/users/${userId}`, { method: 'GET' }));
      const mediaUserData = mediaUserRes.ok ? (await mediaUserRes.json() as { user?: UserProfile }) : { user: undefined };
      const media = (mediaUserData.user?.mediaUrls as Array<{ url: string; type: string }> | undefined) ?? [];

      if (media.length === 0) {
        const keyboard = new InlineKeyboard()
          .text('📤 Upload Media', 'media:upload').row()
      ...
      const lines = media.map((m, i) => `${i + 1}. ${m.type === 'image' ? t('mediaManagerItemPhoto', lang) : t('mediaManagerItemVideo', lang)}`);
...
export async function handleMediaCallback(ctx: MyContext, env: Env, data: string): Promise<boolean> {
  if (!ctx.from) return false;
  const userId = String(ctx.from.id);

  // Fetch user for language and current media
  const userRes = await env.API_SERVICE.fetch(new Request(`http://api/users/${userId}`, { method: 'GET' }));
  let lang: Language = 'en';
  let media: Array<{ url: string; type: string; uploadedAt: string }> = [];
  if (userRes.ok) {
    const userData = await userRes.json() as { user?: UserProfile };
    lang = (userData.user?.language as Language) ?? 'en';
    media = (userData.user?.mediaUrls as Array<{ url: string; type: string; uploadedAt: string }> | undefined) ?? [];
  }
  ...
  if (data.startsWith('media:delete:')) {
    ...
    if (!deleteRes.ok) {
      await ctx.answerCallbackQuery(t('mediaDeleteError', lang)).catch(() => {});
      return true;
    }

    // Refresh media list from API
    const freshRes = await env.API_SERVICE.fetch(new Request(`http://api/users/${userId}`, { method: 'GET' }));
    let freshMedia: Array<{ url: string; type: string }> = [];
    if (freshRes.ok) {
      const freshData = await freshRes.json() as { user?: UserProfile };
      freshMedia = (freshData.user?.mediaUrls as Array<{ url: string; type: string }> | undefined) ?? [];
    }
    ...
    if (freshMedia.length === 0) {
      ...
      const lines = freshMedia.map((m, i) => `${i + 1}. ${m.type === 'image' ? t('mediaManagerItemPhoto', lang) : t('mediaManagerItemVideo', lang)}`);
    ...
    await ctx.answerCallbackQuery(t('mediaDeleteSuccess', lang)).catch(() => {});
    return true;
  }
