...
class TestUserService:
    ...
    def test_set_user_sleeping(self, user_service_module, mock_db_module, sample_user):
        """Test set_user_sleeping."""
        with patch.object(user_service_module, "get_user", return_value=sample_user):
            mock_result = MagicMock()
            mock_result.data = [sample_user.model_dump()]
            mock_db_module.return_value = mock_result

            user_service_module.set_user_sleeping("123", True)

            mock_db_module.assert_called_once()
            # Verify is_sleeping was passed in the update data
            call_args = mock_db_module.call_args
            assert call_args[1]["data"]["is_sleeping"] is True

    def test_set_user_sleeping_wake_up(self, user_service_module, mock_db_module, sample_user):
        """Test set_user_sleeping to wake up."""
        with patch.object(user_service_module, "get_user", return_value=sample_user):
            mock_result = MagicMock()
            mock_result.data = [sample_user.model_dump()]
            mock_db_module.return_value = mock_result

            user_service_module.set_user_sleeping("123", False)

            mock_db_module.assert_called_once()
            call_args = mock_db_module.call_args
            assert call_args[1]["data"]["is_sleeping"] is False

    def test_wake_user(self, user_service_module, mock_db_module, sample_user):
        """Test wake_user updates both is_sleeping and last_active."""
        with patch.object(user_service_module, "get_user", return_value=sample_user):
            mock_result = MagicMock()
            mock_result.data = [sample_user.model_dump()]
            mock_db_module.return_value = mock_result

            user_service_module.wake_user("123")

            mock_db_module.assert_called_once()
            call_args = mock_db_module.call_args
            assert call_args[1]["data"]["is_sleeping"] is False
            assert "last_active" in call_args[1]["data"]

    def test_get_users_for_auto_sleep(self, user_service_module, mock_db_module):
        """Test get_users_for_auto_sleep."""
        ...
        users = user_service_module.get_users_for_auto_sleep(15)

        mock_db_module.assert_called_once()
        assert len(users) == 1
        assert users[0].id == "123"

    def test_get_users_for_auto_sleep_no_results(self, user_service_module, mock_db_module):
        """Test get_users_for_auto_sleep with no inactive users."""
        mock_result = MagicMock()
        mock_result.data = []
        mock_db_module.return_value = mock_result
