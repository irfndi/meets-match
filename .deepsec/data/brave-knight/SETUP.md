# Agent setup for `brave-knight`

This is a deepsec scanning workspace. Project `brave-knight` was just registered
(target: `..`). Setup is incomplete — `data/brave-knight/INFO.md`
...
2. **Fill in `data/brave-knight/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id brave-knight
pnpm deepsec process --project-id brave-knight
```
