...
Currently configured project: `brave-knight` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/brave-knight/SETUP.md` to fill in
   `data/brave-knight/INFO.md`.
...
`--project-id brave-knight` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/brave-knight/`.
