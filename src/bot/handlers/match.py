...
async def get_and_show_match(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: str) -> bool:
    ...
    try:
        ...
        # Format interests
        interests_text = ", ".join([escape_html(i) for i in match_user.interests]) if match_user.interests else "None"

        # Format location
        location_text = (
            f"{escape_html(match_user.location.city)}, {escape_html(match_user.location.country)}"
        ...
        profile_text = MATCH_PROFILE_TEMPLATE.format(
            ...
            age=match_user.age,
            gender=escape_html(match_user.gender.value if match_user.gender else "Not specified"),
